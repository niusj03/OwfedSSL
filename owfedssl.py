...
def args_setting():
    ...
    parser.add_argument('--exp_root', type=str, default='./results/results_cifar10/')
...
def train(args, model, device, train_label_loader, train_unlabel_loader, optimizer, m, epoch, tf_writer, client_id, global_round): # m refers to mean_uncert
    ...
    if is_track:
        ...
        L_clusters, unlabel_ces = AverageMeter('L_cluster', ':.4e'), AverageMeter('unlabel_ce', ':.4e')
    ...
    for batch_idx, ((x, x2), target) in enumerate(train_label_loader):
        ...
        ## the uncertainty weights (fixed) 各个类的不确定性权重（固定值）
        beta = 0.2
        ...
        if is_track:
            ...
            L_clusters.update(L_cluster.item(), args.batch_size)
            unlabel_ces.update(unlabel_ce_loss.item(), args.batch_size)
        # ------------------------------ Finish update ------------------------------
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    # tf_writer.add_scalars('client{}/loss/bce'.format(client_id), {"bce": bce_losses.avg}, args.epochs * global_round + epoch)
    # tf_writer.add_scalars('client{}/loss/ce'.format(client_id), {"ce": ce_losses.avg}, args.epochs * global_round + epoch)
    # tf_writer.add_scalars('client{}/loss/entropy'.format(client_id), {"entropy": entropy_losses.avg}, args.epochs * global_round + epoch)
